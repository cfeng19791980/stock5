# Graph Report - stock5  (2026-05-27)

## Corpus Check
- 23 files · ~21,035 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 362 nodes · 433 edges · 24 communities (22 shown, 2 thin omitted)
...
1. `Stock5LauncherGUI` - 21 edges
2. `Stock5 - 5分钟流式数据分析系统` - 16 edges
3. `parameters` - 9 edges
4. `ModernButton` - 9 edges
5. `meta` - 9 edges
6. `get_fusion_score()` - 9 edges
7. `run_once()` - 9 edges
8. `technical_indicators` - 7 edges
9. `model_v5` - 7 edges
10. `Stock5 分析模型v5 评估报告` - 7 edges
...
Cohesion: 0.06
Nodes (22): extract_minute_5_features(), predict_minute_5(), predict_minute_5_single(), 预测单只股票的5分钟走势          Args:         code: 股票代码          Returns:         d, 预测多只股票的5分钟走势          Args:         codes: 股票代码列表          Returns:, 从minute_5_price表提取5分钟周期特征          Args:         code: 股票代码         conn: 数据, run_minute_5_analysis(), save_predictions_to_db() (+14 more)

### Community 1 - "Community 1"
Cohesion: 0.07
Nodes (28): database, path, tables, llm, description, enabled, model, port (+20 more)
...
Cohesion: 0.07
Nodes (26): 1. 简化架构, 2. 技术指标适配, 3. 流式实时分析, code:block1 (stock5/), code:block6 (腾讯API实时数据（每5分钟）), KDJ指标（基于9个5分钟周期）, MACD指标（基于26个5分钟周期）, MA均线（基于5分钟周期） (+18 more)
...
Cohesion: 0.11
Nodes (19): calc_anomaly_score(), calc_llm_insight_score(), get_anomaly_signals(), get_fundamental_signals(), get_fusion_score(), get_tech_score(), 从 daily_features 表获取异动和 LLM 分析数据。, 从数据库获取当天因子信号。          资金流向信号从 fund_flow 表读取（实时采集），     不再依赖 factor_signals 的 (+11 more)
...
Cohesion: 0.21
Nodes (15): analyze_news_llm(), calc_fund_score(), call_llm(), collect_fund(), collect_fundamental(), collect_macro(), collect_news(), daemon_loop() (+7 more)
...
Cohesion: 0.22
Nodes (9): 1. 数据采集（单次）, 2. 数据采集（后台连续）, 3. 模型分析（单次）, 4. 使用启动脚本, code:bash (python realtime_fetcher.py --once), code:bash (python realtime_fetcher.py --daemon --interval 5), code:bash (python analyzer_v5.py), code:bash (启动_数据采集.bat  # 启动后台数据采集) (+1 more)

### Community 16 - "Community 16"
Cohesion: 0.25
Nodes (8): data_collection, frequency_minutes, source, stock_count, stock_pool, trading_hours, afternoon, morning
...
## Knowledge Gaps
- **136 isolated node(s):** `name`, `version`, `description`, `created`, `path` (+131 more)
...
- **Why does `Stock5 - 5分钟流式数据分析系统` connect `Community 4` to `Community 15`?**
  _High betweenness centrality (0.008) - this node is a cross-community bridge._
- **What connects `预加载大盘/板块/基本面因子，按日期索引，覆盖全历史`, `从minute_5_price表提取5分钟周期特征          Args:         code: 股票代码         conn: 数据`, `预测单只股票的5分钟走势          Args:         code: 股票代码          Returns:         d` to the rest of the system?**
