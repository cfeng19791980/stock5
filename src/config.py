...
# Paths (can be overridden via env) — defaults relative to this file
import pathlib
_RUN_DIR = pathlib.Path(__file__).parent.absolute()
DB_PATH = getenv("STOCK5_DB", str(_RUN_DIR / "stocks.db"))
CSV_PATH = getenv("STOCK5_CSV", str(_RUN_DIR / "波段股票Top30.csv"))
MODEL_CACHE_DIR = getenv("STOCK5_CACHE", str(_RUN_DIR / "v6" / "model_cache_v6"))
OUTPUT_JSON = getenv("STOCK5_OUTPUT", str(_RUN_DIR / "result_v6.json"))
